...
if resp.status_code == 202 and loc:
    ...
    success = False
    for attempt in range(1, 61):  # ~5 minutes (61 * 5s)
        time.sleep(5)
        jr = requests.get(loc, headers=hdr, timeout=30)
        code = jr.status_code
        txt = jr.text or ""
        # print a short debug line each poll; include status-like fields if present in JSON
        try:
            j = jr.json()
        except Exception:
            j = None
...
# also write datasets.json derived from params (so upload-artifact step has something)
datasets = []
